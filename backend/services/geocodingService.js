...
/**
 * Reverse Geocode: Get address & clean name from latitude & longitude
 */
const reverseGeocode = async (latitude, longitude) => {
  ...
  if (geoapifyKey && geoapifyKey !== 'YOUR_KEY') {
    try {
      ...
      if (geoRes.data?.features && geoRes.data.features.length > 0) {
        const feat = geoRes.data.features[0];
        const props = feat.properties;
        const shortName = props.city || props.suburb || props.name || props.formatted.split(',')[0];
        return {
          name: props.formatted,
          shortName: shortName,
          latitude: lat,
      ...
      console.warn(`[Reverse Geocode] Geoapify notice: ${err.message}. Falling back to OpenStreetMap.`);
    }
  }

  // 2. Try OpenStreetMap Reverse Geocode
  try {
  ...
  const approxName =
    nearestCity && minDistance < 1.0
      ? `Near ${nearestCity.shortName} (${lat.toFixed(4)}°, ${lon.toFixed(4)}°)`
